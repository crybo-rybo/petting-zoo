<script lang="ts">
  ...
  import type { ChatMessage as ChatMessageType, ModelSummary } from './shared/api/types';
  ...
  async function unloadModel() {
    ...
    try {
      ...
      activeModelId = null;
      isModelModalOpen = false;
      liveAnnouncement = 'Model unloaded';
    } catch (e) {
      apiError = e instanceof Error ? e.message : 'unknown error';
    } finally {
      busy = false;
  ...
  async function sendChat() {
    ...
    isResponseTopOffscreen = false;
    liveAnnouncement = 'Assistant response started';

    abortController = new AbortController();
    let assistantIndex: number | null = null;

    try {
      const responseBody = await openChatStream(userMessage, abortController.signal);

      await consumeSseStream(responseBody, async (event) => {
        if (event.type === 'token' && event.content !== undefined) {
          if (assistantIndex === null) {
            chatHistory = [...chatHistory, { role: 'assistant', content: event.content }];
            assistantIndex = chatHistory.length - 1;
          } else {
            chatHistory[assistantIndex].content += event.content;
      ...
      if (e instanceof Error && e.name === 'AbortError') {
        ...
        if (assistantIndex !== null) {
          ...
          chatHistory = [...chatHistory, { role: 'assistant', content: '*(Generation stopped)*' }];
        }
        liveAnnouncement = 'Generation stopped';
      } else {
        chatError = e instanceof Error ? e.message : 'unknown error';
        if (assistantIndex !== null && !chatHistory[assistantIndex].content.trim()) {
          chatHistory = chatHistory.filter((_, idx) => idx !== assistantIndex);
        }
      }
    } finally {
      busy = false;
...
<main class="shell">
  ...
  <div class="top-tabs">
    <button class="ghost tab-btn" type="button" on:click={() => (isModelModalOpen = true)}>
      ...
      <div class="tab-info">
        ...
        <span class="tab-summary">
          {#if activeModelId}
            Active:
            <span class="mono accent-text" title={activeModelId}>
              {availableModels.find((m) => m.id === activeModelId)?.display_name ?? activeModelId}
            </span>
          {:else}
            No model loaded
          {/if}
  ...
  <ModelConfigModal
    ...
    {selectedContextSize}
    {busy}
    on:close={() => (isModelModalOpen = false)}
    on:load={loadAndSelectModel}
    on:unload={unloadModel}
    on:selectedModelIdChange={(event) => (selectedModelId = event.detail)}
    on:selectedContextSizeChange={(event) => (selectedContextSize = event.detail)}
  />
  ...
  <section class="card chat-container glass">
    ...
    {#if isResponseTopOffscreen}
      <button class="jump-btn fade-in" type="button" on:click={jumpToActiveResponse} title="Jump to top of recent response">
        ↑ Top of Response
      </button>
    {/if}
    ...
    <div class="chat-history" bind:this={chatContainer} on:scroll={handleScroll}>
      ...
      {:else}
        {#each chatHistory as msg, i (i)}
          <ChatMessage message={msg} index={i} isStreaming={chatStreaming} isLast={i === chatHistory.length - 1} onCopy={handleCopy} />
        {/each}
    ...
    <ChatComposer
      ...
      {chatStreaming}
      maxChars={MAX_CHAT_INPUT_CHARS}
      canReset={chatHistory.length > 0}
      on:send={sendChat}
  ...
  {#if apiError}
    <section class="card glass error-card slide-up">
      <ErrorBanner prefix="API Error" message={apiError} />
    </section>
  {/if}
</main>
<style>
  ...
  /* ---- INPUTS & BUTTONS ---- */
  :global(.model-input-row) {
    display: flex;
    gap: 0.75rem;
    align-items: stretch;
  }

  :global(select.model-select) {
    font-family: inherit;
    font-size: 0.95rem;
    font-weight: 600;
    background: #ffffff;
  ...
  :global(select.model-select:focus) {
    outline: none;
  ...
  :global(select.model-select:disabled) {
    opacity: 0.6;
    cursor: not-allowed;
    background: #ebe6d8;
  }

  :global(.context-size-row) {
    display: flex;
    align-items: center;
    gap: 0.75rem;
    margin-top: 0.75rem;
  ...
  :global(select.context-select:focus) {
    outline: none;
    border-color: var(--accent-orange);
    box-shadow: 2px 2px 0px var(--accent-orange);
  }

  :global(select.context-select:disabled) {
    opacity: 0.6;
    cursor: not-allowed;
    background: #ebe6d8;
  }
  ...
  :global(button.glow:not(:disabled):hover) {
  ...
  :global(button.danger.ghost) {
    color: var(--danger);
    border-color: var(--danger);
    box-shadow: 3px 3px 0px var(--danger);
  }
  
  :global(button.danger.ghost:not(:disabled):hover) {
    background: var(--danger-muted);
  }
  
  :global(button.danger.ghost:active:not(:disabled)) {
    transform: translate(3px, 3px);
  ...
  :global(.message.user .message-header) {
    color: #fff;
    opacity: 0.8;
  }
  ...
  :global(.chat-input-row textarea) {
    flex: 1;
    resize: none;
    min-height: 50px;
  }
  ...
  :global(.error) { color: var(--danger); font-weight: 500; }
